...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        customtkinter.CTkButton(self.sidebar, text="Timer", command=self.show_timer).pack(pady=10, padx=10, fill="x")
        customtkinter.CTkButton(self.sidebar, text="Tasks", command=self.show_tasks).pack(pady=10, padx=10, fill="x")
        customtkinter.CTkButton(self.sidebar, text="Budget", command=self.show_budget).pack(pady=10, padx=10, fill="x")
        customtkinter.CTkButton(self.sidebar, text="Calendar", command=self.show_calendar).pack(pady=10, padx=10, fill="x")
    ...
    def run_timer(self):
        while self.time_left >= 0:
            ...
            self.time_left -= 1
        self.timer_running = False
        self.timer_status.configure(text="Time's up! Great job focusing.")
    ...
    def show_tasks(self):
        ...
        self.task_name_input = customtkinter.CTkEntry(form, width=150)
        ...
        self.task_category_input = customtkinter.CTkEntry(form, width=150)
        ...
        customtkinter.CTkButton(form, text="Add Task", command=self.add_task).grid(row=1, column=3, padx=5, pady=5)
    ...
    def add_task(self):
        ...
        if not name:
            ...
            return
        self.tasks.append({"Name": name, "Category": category, "Difficulty": difficulty, "Done": False})
    ...
    def refresh_task_list(self):
        ...
        for i, task in enumerate(self.tasks):
            ...
            customtkinter.CTkLabel(row, text=f"{status} {task['Name']} | {task['Category']} | Difficulty: {task['Difficulty']}").pack(side="left", padx=10)
    ...
    def refresh_budget_list(self):
        ...
        self.budget_summary.configure(text=f"Total Income: {total_income} | Total Expenses: {total_expense} | Balance: {total_income - total_expense}")
    ...
    def open_day(self, day):
        ...
        if day_sessions:
            for s in day_sessions:
                customtkinter.CTkLabel(scroll, text=f"  {s['Minutes']} minutes").pack(anchor="w", padx=10)
        else:
            customtkinter.CTkLabel(scroll, text="  None").pack(anchor="w", padx=10)

        # tasks
        customtkinter.CTkLabel(scroll, text="Tasks:", font=("Arial", 14)).pack(anchor="w", pady=(10,0))
        if self.tasks:
            for task in self.tasks:
                status = "✓" if task["Done"] else "✗"
                customtkinter.CTkLabel(scroll, text=f"  {status} {task['Name']} | {task['Category']} | Difficulty: {task['Difficulty']}").pack(anchor="w", padx=10)
        else:
            customtkinter.CTkLabel(scroll, text="  None").pack(anchor="w", padx=10)
